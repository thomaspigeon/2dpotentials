...
class CommittorOneDecoder(torch.nn.Module):
    ...
    def xi_forward(self,  X):
        ...
        return self.committor(X.to(device)).cpu().detach().numpy()
...
class CommittorTwoDecoder(torch.nn.Module):
    ...
    def inR(self, inp):
        return 1 - (1 / 2 + (1 / 2) * self.HT(
            (2 * (1 / self.boundary_width) * (torch.sqrt(torch.sum((inp[:, :2] - self.minR) ** 2, dim=1)).reshape(
                [len(inp), 1]) - self.R_radius) / self.R_radius) + 1))

    def inP(self, inp):
        return 1 - (1 / 2 + (1 / 2) * self.HT(
            (2 * (1 / self.boundary_width) * (torch.sqrt(torch.sum((inp[:, :2] - self.minP) ** 2, dim=1)).reshape(
...
class CommittorUnderdampedOneDecoder(torch.nn.Module):
    def __init__(self, committor_dims, decoder_dims, dropout, pot, boundary_width=0.1, handtanh=True):
        """Initialise auto encoder with hyperbolic tangent activation function
